...
import numpy as np
import math
from layers.Transformer_EncDec import Decoder, DecoderLayer, Encoder, EncoderLayer, ConvLayer,DecoderLayer_ETT
...
class single_scale_gnn(nn.Module):
    def __init__(self, configs):
        ...
        self.time_con = nn.Linear(self.seq_len,self.init_seq_len)
    ...
    def add_cross_var_adj(self,adj):
        k = 3
        if self.configs.enc_in < 100:
            # k = max(k,int(0.3*adj.shape[0]))
            k = self.configs.enc_in
        else:
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        # x = self.time_con(x.permute(0,2,1)).permute(0,2,1)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.rmsnorm2 = RMSNorm(configs.d_model)
        self.rmsnorm3 = RMSNorm(configs.pred_len)
        self.rmsnorm4 = RMSNorm(configs.pred_len)
        self.rmsnorm5 = RMSNorm(configs.c_out)

        self.weight = nn.Parameter(torch.ones(configs.c_out))
        self.weight_1 = nn.Parameter(torch.ones(configs.c_out))
        self.weight_time = nn.Parameter(torch.ones(configs.pred_len))

        self.weight_linear_1 = nn.Linear(configs.seq_len, configs.pred_len)
        self.weight_linear_2 = nn.Linear(configs.seq_len, configs.pred_len)
    def forward(self,x, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        # print(mean_enc.shape,std_enc.shape)
        # B x S x E, B x 1 x E -> B x 1, positive scalar
        
        tau = None
        delta = None
        # tau = self.tau_learner(x, std_enc).exp()
        # B x S x E, B x 1 x E -> B x S
        # delta = self.delta_learner(x, mean_enc)

        # x_raw = self.rmsnorm(x_raw)
        g_x = self.enc_embedding(x,None)
        ...
        pred_x = x
        # pred_x = self.rmsnorm5(pred_x)
        pred_x = pred_x[:, -self.pred_len:, :] 

     
        
        pred_x_g = self.EtG(self.Linear(g_x.permute(0,2,1)).permute(0,2,1))
        # pred_x_g =self.rmsnorm4(pred_x_g.permute(0,2,1)).permute(0,2,1)
        ...
        # pred_x = pred_x * self.weight_linear_1(std_enc) + self.weight_linear_2(mean_enc)
        # pred_x = pred_x * std_enc + mean_enc
        # pred_x = self.rmsnorm3(pred_x)
        # pred_x = pred_x.permute(0,2,1)  

        # pred_x = pred_x  + mean_enc
        # pred_x = self.rmsnorm4(pred_x)
        # pred_x = self.weight_1 *  pred_x /(self.weight_1+self.weight) +  self.weight * pred_x_g / (self.weight_1+self.weight)
        pred_x = self.weight_1 *  pred_x +  self.weight * pred_x_g
